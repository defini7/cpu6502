...
| to another. HOWEVER there is a problem: what if we want to jump backwards?
| Then we need to somehow specify an unsigned number and the solution for that is
| to convert a range [0, 255] to [-128, 127]
*/
bool CPU6502::REL()
{
	rel_addr = read(program_counter++);

	if (rel_addr & 0b10000000) // rel_addr > 127
	{
		// If the first bit is set to 1 then we have to treat
		// that offset as a negative value so we need to sign-extend it
		// by setting the first byte to $FF so after performing an addition
		// with the program counter the overflow of the uint16_t, i.e. of the program counter,
		// will occur and the program counter will wrap around, i.e. it will "jump backwards".
		// Very smart, huh?
		rel_addr |= 0xFF00;

		// Example: we want to jump backwards by 9 so we use $F7
		// as an offset, $F7 is $00F7 and it becomes $FFF7 so then
		// we perform pc = pc + $FFF7 and the overflow occurs because
		// the maximum possible value of the PC is $FFFF and we get new_pc = old_pc - $9
	}
...
bool CPU6502::IND()
{
	...
	// One crucial moment: if the ptr occurs to be $xxFF then
	// we read the high byte of the abs_addr at ($xxFF + $1)
	// so we cross the current page but there is a hardware
	// bug in the CPU that doesn't allow us to move to
	// the next page if there is a page crossing

	// Reference: https://www.nesdev.org/6502bugs.txt

	// Simulating the real behaviour

	// Ignore the page crossing and just wrap around
	...
	// But we still apply an offset on the current page
	abs_addr_1 = read(ptr);
...
/* ADd with a Carry
| Simly performs an addition. However, it is not as easy as it sounds:
...
bool CPU6502::SBC()
{
	...
	// ~M
	uint16_t inv = (uint16_t)memory ^ 0x00FF; // + 1 - 1

	// A + ~M + C
	uint16_t res = (uint16_t)accumulator + inv;
	if (get_flag(flag_c)) res++;
