    ...
    void disassemble()
    {
        ...
        while (addr <= program_end)
        {
            ...
            if (inst_info.addr_mode == &CPU6502::IMP)
                inst << " {IMP}";
            if (inst_info.addr_mode == &CPU6502::IMM)
            {
...
protected:
    bool OnUserCreate() override
    {
        ...
        /*
        LDA #$10        ; Load accumulator with $10 (16 in decimal)
        CLC             ; Clear carry flag (important before ADC)
        ADC #$20        ; Add $20 (32 in decimal) to accumulator
        STA $0000       ; Store result at memory location $0000
        */

        std::stringstream code;
        code << "A9 10 18 69 20 8D 00 00";
